...
public class VedioChat {
    ...
    public void onMessage(String message, Session session, @PathParam("uid") String uid) {
        try {
            if (uid != null && user_user.get(uid) != null && VedioChat.sessions.get(user_user.get(uid)) != null) {
    ...
    public static boolean addUser(String uid, String oid) {
        if (oid != null && !oid.isEmpty()) {
            VedioChat.user_user.put(uid, oid);
            VedioChat.user_user.put(oid, uid);

            return false;
        } else {
            VedioChat.user_user.put(uid, null);
